...
- 앱 삭제, 기기 분실, Keystore 손실 시 설치 신원과 로컬 메시지를 복구하지 못할 수 있다. 1.0은 cloud key backup을 제공하지 않는다.
- Android 제조사 절전 정책에 따라 background relay가 중단될 수 있으며 지원 기기 목록은 release별 evidence를 기준으로 한다.
- BLE 전파 방해, 악성 대량 노드, 물리적 기기 탈취, OS compromise를 완전히 방어하지 못한다.
- 앱은 공식 112/119 신고 수단이 아니며 긴급 구조 접수·도착 시간을 보장하지 않는다.
- store 배포 가능성과 background permission 승인은 국가·스토어 정책에 따라 달라질 수 있다.
