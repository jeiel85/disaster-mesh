...
이 값들은 릴레이에서 합법적으로 변한다. `hop_limit`은 생성 후 변경할 수 없는 값이므로 반드시 AAD에 포함한다. AAD의 byte-exact 형식은 `spec/dme-aad-v1.cddl`이 기준이다.
...
| 수신 message type | receipt 생성 |
|---|---|
| DIRECT_TEXT | 1회 생성 |
| CHECK_IN | 1회 생성 |
| PRIVATE_SOS | 1회 생성 |
| LOCATION_UPDATE | 1회 생성 |
| DELIVERY_RECEIPT | **절대 생성하지 않음** |
| CANCEL | cancel 자체의 수신 확인이 필요할 때 1회 생성 |

CANCEL에 대한 receipt는 `original_packet_id/original_message_id`가 cancel packet/message를 가리킨다. 그 receipt는 다시 receipt를 만들지 않는다. `should_generate_receipt(message_type)`는 pure function이며 exhaustive test 대상이다.
...
- 수신자는 contact별 `max_sender_sequence`와 4096-bit persisted sliding bitmap을 저장한다.
- `seq > max`: bitmap을 차이만큼 shift하고 새 max bit를 set한다. 차이가 4096 이상이면 bitmap을 clear한다.
- `max-4095 <= seq <= max`: 대응 bit가 1이면 replay, 0이면 DTN 지연 도착으로 수락 후 bit를 set한다.
- `seq < max-4095`: `STALE_REPLAY_SUSPECTED`로 격리한다.
- 동일 message ID/packet ID는 sequence bitmap과 별개로 중복 거부한다.
- replay state update와 inbound message commit은 같은 DB transaction에서 수행한다.
